...
num_iterations_taken = np.full((len(RECOVERY_EFFICIENCIES), NUM_POINTS_ON_XAXIS), np.nan, dtype=np.int)

for recovery_efficiency_i, recovery_efficiency in enumerate(RECOVERY_EFFICIENCIES):
    for kd_i, kd in enumerate(ligand_kd_range):
        num_iterations_taken[recovery_efficiency_i,kd_i] = len(pbSEC_iterate_until_undetectable(
            PROTEIN_CONC, LIGAND_CONC, kd, L_DETECTION_LIMIT, recovery_efficiency=recovery_efficiency))

fig, ax = plt.subplots(1, 1, figsize=(7.204724, 5.09424929292))

for recovery_efficiency_i, recovery_efficiency in enumerate(RECOVERY_EFFICIENCIES):
    ax.plot(x_axis, num_iterations_taken[recovery_efficiency_i], label=f"{recovery_efficiency*100:2.0f} % recovery")
# ax.hlines(1,3,9)
plt.legend()
